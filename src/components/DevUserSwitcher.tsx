...
function setDevIdentity(id: string, name?: string) {
  ...
  // keep admin off by default; we toggle it separately
...
export function DevUserSwitcher() {
  ...
  const [isAdmin, setIsAdmin] = useState<boolean>(
    typeof window !== "undefined" ? localStorage.getItem("is_admin") === "1" : false
  );
  ...
  return (
    <div className="fixed bottom-3 right-3 z-50 rounded-xl border border-slate-200 bg-white/95 shadow-lg p-2 text-xs">
      ...
      <div className="flex gap-1 flex-wrap mb-2">
        ...
        <button onClick={clearUser} className="px-2 py-1 rounded border">
          Clear
        </button>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={
            "px-2 py-1 rounded border " +
            (isAdmin ? "bg-emerald-600 text-white border-emerald-700" : "bg-white")
          }
          title="Toggle admin role locally"
        >
